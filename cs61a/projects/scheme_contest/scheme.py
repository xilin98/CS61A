...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure, MacroProcedure):
            remai_evaluate = procedure.apply_macro(rest, env)
            return scheme_eval(remai_evaluate, env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        while not (formals is nil and vals is nil):
            if formals is nil and vals is not nil:
                raise SchemeError("Too much arguments values were given")
            elif formals is not nil and vals is nil:
                raise SchemeError("Too less argements values were given")
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("The wrong number of arguments was passed")
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        v = do_lambda_form(Pair(target.second, expressions.second), env)
        env.define(target.first, v)
        return target.first
        # END PROBLEM 10
    else:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            "*** YOUR CODE HERE ***"
            while clause.second is not nil:
                test = scheme_eval(clause.second.first, env, True)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        v = MacroProcedure(expressions.first.second, expressions.second, env)
        env.define(target.first, v)
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
