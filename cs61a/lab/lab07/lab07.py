...
def store_digits(n):
    ...
    def inverse(n):
        a,rest=n%10, n//10
        if rest:
            return a*(10**count(rest))+inverse(rest)
        else:
            return n
    rever=inverse(n)
    def inner(rever):
        a, rest = rever%10, rever//10
        if rest:
            return Link(a, inner(rest))
        else:
            return Link(a)
    return inner(rever)
    # My stupid solution , but I figure it out !!!
    # NEW method:
    """
    def invese(n):
        rev=0
...
def store_digits(n):
    if n==0:
        ...
        return Link(n//(10**(len(n)-1)),store_digits(n%(10**(len(n)-1))))
    """
# Q6
def cumulative_sum(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    #if t.is_leaf:       #  Missed the ()
    #    pass            #  All of these is useless
    for b in t.branches:
        cumulative_sum(b)
        t.label+=b.label
